...
// (automatically produced by ROOT, then slightly tuned manually)
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
public :
   ...
   Float_t         jetBTagDiscr[maxNjet];   //[Njet] jet b-tagging discriminant (CSV)
   Float_t         jetBTagMatchDiff1[maxNjet];   //[Njet] (not used, for checks)
   Float_t         jetBTagMatchDiff2[maxNjet];   //[Njet] (not used, for checks)
   ...
   Float_t mcPhiTT; // phi angle between top and antitop (new)
...
void ZTree::Init(TTree *tree)
{
   ...
   if(_flagMC) fChain->SetBranchAddress("mcPhiTT", &mcPhiTT, &b_mcPhiTT);
}
